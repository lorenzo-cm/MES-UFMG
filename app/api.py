...
from utils import validate_email, sanitize_string, validate_task_input
import json
...
class TaskAPI:
    ...
    def create_task(self, title: str, description: str, assigned_to_id: Optional[int] = None, priority: int = 2) -> Dict:
        is_valid, error_message = validate_task_input(title, description, priority, assigned_to_id)
        if not is_valid:
